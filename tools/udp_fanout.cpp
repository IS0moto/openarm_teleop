...
void print_usage(const char* argv0) {
    std::cerr
        ...
        << "\n"
        << "Example:\n"
        << "  " << argv0 << " --bind-ip 0.0.0.0 --listen-port 54002 "
        << "--target 127.0.0.1:54012 --target 127.0.0.1:54022\n";
}
...
int main(int argc, char** argv) {
    ...
    std::cout << "UDP fanout listening on " << bind_ip << ":" << listen_port << "\n";
    ...
    while (keep_running) {
        ...
        if (log_debug && packets % 100 == 0) {
            std::cout << "[DEBUG] Forwarded " << packets << " UDP packets\n";
        }
    }
